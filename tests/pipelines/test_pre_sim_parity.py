...
from microplex_us.pipelines.pre_sim_parity import build_us_pre_sim_parity_audit
...
def test_build_us_pre_sim_parity_audit_reports_schema_and_support(tmp_path) -> None:
    ...
    audit = build_us_pre_sim_parity_audit(
        candidate_path,
        reference_path,
        focus_variables=("age", "state_fips", "county_fips"),
        critical_reference_variables=("county_fips",),
    )

    assert audit["schema"]["reference_variable_count"] == 11
    assert audit["schema"]["candidate_variable_count"] == 9
